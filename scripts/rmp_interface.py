...
class RMP:
    ...
    def set_and_verify_config_params(self,config):
        ...
        if (False == success):
            ...
            for i in range(0,NUMBER_OF_NVM_CONFIG_PARAMS):
                if (loaded_params[i] != config[i][2]):
                    ...
                    print "non matching %(1)s " %{"1":config_param_dict[i+1]}
                    print "%(1)s vs. configured %(2)s" %{"1":loaded_params[i], "2":config[i][2]}
                else:
		    print "matching %(1)s " %{"1":config_param_dict[i+1]}
        ...
        for i in range(0,len(non_matching_params)):
            ...
            print "setting %(1)s " %{"1":config_param_dict[idx+1]}
            success = False
            while ((False == success) and (attempts<10)):
                attempts+=1
